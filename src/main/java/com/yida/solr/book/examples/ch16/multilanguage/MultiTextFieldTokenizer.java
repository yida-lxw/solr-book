...
public class MultiTextFieldTokenizer extends Tokenizer {
    ...
    protected MultiTextFieldTokenizer(IndexSchema indexSchema,
    ...
    private LinkedHashMap<String, Analyzer> getNamedAnalyzers() {
        ...
        for (int i = 0; i < this.multiTextInput.Keys.size(); i++) {
            ...
            if (fieldType != null) {
                if (this.settings.analyzerMode == MultiTextFieldSettings.AnalyzerModes.query) {
                    namedAnalyzers.put(fieldTypeName, fieldType.getQueryAnalyzer());
                } else if (this.settings.analyzerMode == MultiTextFieldSettings.AnalyzerModes.multiTerm) {
                    namedAnalyzers.put(fieldTypeName,
                            ((TextField) fieldType).getMultiTermAnalyzer());
                } else {
                    namedAnalyzers.put(fieldTypeName, fieldType.getIndexAnalyzer());
                }
            } else {
            ...
            if (this.settings.defaultFieldTypeName != null
                ...
                if (this.settings.analyzerMode == MultiTextFieldSettings.AnalyzerModes.query) {
                    ...
                    namedAnalyzers.put(
                            "",
                            this.indexSchema.getFieldTypeByName(
                                    this.settings.defaultFieldTypeName).getIndexAnalyzer());
                }
            }
    ...
    private void addTokenStreamForFieldType(
        ...
        for (boolean hasMoreTokens = tokenStream.incrementToken(); hasMoreTokens; hasMoreTokens = tokenStream
            ...
            if (charTermAtt == null
                    || offsetAtt == null
                    || (typeAtt == null && this.settings.analyzerMode != MultiTextFieldSettings.AnalyzerModes.multiTerm)) {
                return;
            }
            ...
            clone.setType(multiTermSafeType);
